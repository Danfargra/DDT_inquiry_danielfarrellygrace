...
Adds two login paths on top of database.py:
  ...
  1. Parent / guardian login  — username + password, for the person who
     manages balances and transactions (the "admin" of the app).

  2. Child quick-login        — a short PIN, so kids can open their own
     balance/transaction view without typing a full password.

Passwords and PINs are never stored in plain text. They are hashed with
