...
#functions used from Operating System

DB_PATH = os.path.join(os.path.dirname(__file__), "data", "ranui.db")
#Use only one database for everything. (I can create many tables)

#'bridge' connection: how you python talks to your database
def get_connection():
    #dir = directory
    os.makedirs(os.path.dirname(DB_PATH), exist_ok=True)
    return sqlite3.connect(DB_PATH) #bridge between two files

#initiate database through init_db
def init_db():
    """Create tables if they don't exist."""
    with get_connection() as conn:
        cur = conn.cursor() #pointer/cursor that moves in tables
        ...
        if cur.fetchone()[0] == 0:
            for name in ["Nikau", "Hana", "Tia"]: #putting values into the table --- names are hard coded
